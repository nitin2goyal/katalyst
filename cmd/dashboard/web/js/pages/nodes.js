...
export async function renderNodes(targetEl) {
  ...
  try {
    ...
    container().innerHTML = `
      ...
      <div class="card">
        ...
        <div class="table-wrap"><table id="ng-table">
          <thead><tr><th>Name</th><th>Instance Type</th><th>Family</th><th>Count</th><th>Min</th><th>Max</th><th>Total Cores</th><th>Total Memory</th><th>CPU Util</th><th>Mem Util</th><th>CPU Alloc</th><th>Mem Alloc</th><th>Cost/mo</th></tr></thead>
    ...
    }).join('') : '<tr><td colspan="13" style="color:var(--text-muted)">No node groups</td></tr>';
    ...
    window.__exportNgCSV = () => {
      exportCSV(['Name', 'Instance Type', 'Family', 'Count', 'Min', 'Max', 'Total Cores', 'Total Memory (GiB)', 'CPU Util %', 'Mem Util %', 'CPU Alloc %', 'Mem Alloc %', 'Cost/mo'],
        ngList.map(ng => [ng.name, ng.instanceType, ng.instanceFamily, ng.currentCount, ng.minCount, ng.maxCount, ng.totalCPU ? (ng.totalCPU / 1000).toFixed(0) : 0, ng.totalMemory ? (ng.totalMemory / (1024*1024*1024)).toFixed(1) : 0, (ng.cpuUtilPct||0).toFixed(1), (ng.memUtilPct||0).toFixed(1), (ng.cpuAllocPct||0).toFixed(1), (ng.memAllocPct||0).toFixed(1), ng.monthlyCostUSD]),
